...
<script>
    ...
    function restor(id, ind) {
        ...
        $('#row' + ind).html(`
                                        <td id="photo${ind}">
                                            <div>
                                                <img class="rounded-circle avatar-xs" src="{{ asset('storage/${jsonData.prospect.photo}') }}" alt="">
                                            </div>
                                        </td>
                                        <td id="nom_prenom${ind}">${jsonData.prospect.nom}  ${jsonData.prospect.prenom}</td>
                                        
                                        <td id="email${ind}">${jsonData.prospect.email}</td>
                                        <td id="role${ind}">${jsonData.prospect.role.value}</td>
                                        <td id="group${ind}">${jsonData.prospect.group.nom}</td>
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                            <button  class="btn btn-success" style="margin: 10px" onclick="detail(${jsonData.prospect.id},${ind})" ">détails</button>
                                            <button  class="btn btn-primary" style="margin: 10px" >Parcourir</button>
