...
<div class="row ">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Superviseur du group</h4>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group" id="err-superviseur_list">
                    <label for="superviseur_list" class="control-label"><b>list des Superviseurs</b></label>
                    <select class="form-control  " name="superviseur_list" id="superviseur_list">
...
<script>
    ...
    function init() {
        ...
        var buttonacive;
        let group_link = window.location.href.split('/')[4];
        var StringData = $.ajax({
            url: 'detailGR/index',
            dataType: "json",
            type: "POST",
    ...
    function attach_sup() {
        ...
        $('#modalhead').html("<h4 class=\"modal-title\" >Attacher superviseur</h4>" +
            "<button type=\"button\" class=\"close\" data-bs-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>");
        $('#modalfooter').html("<button type=\"button\" class=\"btn btn-info\" id=\"save_sup\">Enregistrer</button>");
        $('#err-superviseur_list').show()
        $('#superviseur_list').html("");
        var StringData2 = $.ajax({
            url: "detailGR/list_sup",
            dataType: "json",
            type: "POST",
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        for (let ind = 0; ind < jsonData2.length; ind++) {
            $('#superviseur_list').append("<option value=\"" + jsonData2[ind].id + "\">" + jsonData2[ind].nom + " " + jsonData2[ind].prenom + "</option>");
        }
        $('#err-user_list').hide()
        ...
        $('#save_sup').click(function() {
            var inputs = {
                "new_sup": $("#superviseur_list").val()
            };

            var StringData = $.ajax({
                url: "detailGR/attach_sup",
                type: "POST",
                async: false,
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            $('#supervisor').html(`<tr class="table-success" >
                                        <td >
                                            <div>
                                                <img class="rounded-circle avatar-xs" src="{{ asset('storage/${jsonData.photo}') }}" alt="">
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                            <button  class="btn btn-success" style="margin: 10px" onclick="detail(${jsonData.id},-1)">détails</button>
                                            <button class="btn btn-warning"style="margin: 10px" onclick="attach_sup()">changer superviseur</button>
                                            <button class="btn btn-danger"style="margin: 10px" onclick="dettach_sup()">détacher</button>
    ...
    function dettach_sup() {


        var StringData = $.ajax({
            url: "detailGR/detach_sup",
            dataType: "json",
            type: "POST",
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
